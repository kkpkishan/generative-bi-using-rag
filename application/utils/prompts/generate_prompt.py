...
# knowledge Knowledge base answers prompt

knowledge_system_prompt_dict = {}
knowledge_user_prompt_dict = {}

# agent task Agent task splitting prompt
...
<example>
question : How many hotels does Hilton have online in Europe?
answer :
{
    "intent" : "normal_search",
    "slot" : ["Hilton", "Europe", "Online", "Number of hotels"]
}

question : How many orders did Apple mobile phones have on JD.com in March?
answer :
{
    "intent" : "normal_search",
    "slot" : ["iPhone", "March", "JD.com", "Order"]
}

question : Modify the first row of data in the order table
answer :
{
    "intent" : "reject_search"
}

question : Why did hotel bookings drop in June?
answer :
{
    "intent" : "agent_search"
}
...
question : What is the English name of Hilton?
answer :
{
...
[{'Acronym': 'NDDC', 'Full name': 'Nike Direct Digital Commerce'},
 ...
 {'Acronym': 'etc', 'Full name': 'et cetera', 'Comment': 'means "etc"'},
...
def generate_sagemaker_explain_prompt(
        query: str,
        history=[],
        meta_instruction="You are an AI assistant whose name is InternLM (Scholar Puyu).\n"
                         "- InternLM (Scholar Puyu) is a conversational language model that is developed by Shanghai AI Laboratory (Shanghai Artificial Intelligence Laboratory). It is designed to be helpful, honest, and harmless.\n"
                         "- InternLM (Scholar Puyu) can understand and communicate fluently in the language chosen by the user such as English and 中文.",
):
    prompt = ""
    if meta_instruction:
        prompt += f"""<|im_start|>system\n{meta_instruction}<|im_end|>\n"""
